...
### [sidu-red-river](https://github.com/lengmodkx/sidu-red-river)

同主题的姊妹项目 —— **四渡赤水 · Four Crossings of the Red River**,以更轻量的方式呈现同一段历史:

- **技术栈不同**:Vite 8 + React 19 + TypeScript + framer-motion 12 + **SVG 矢量地图**(无 MapLibre、无真实瓦片、无 3D),可作为本仓方案的低成本替代
- **叙事更轻快**:首屏 Hero + 阶段详图 + 复盘三段式,SVG 视口 1000×700,部队框 + 锚点小三角 + 红线 `pathLength` 动画绘制
- **交互更聚焦**:**可拖动时间轴 + 真插值**(红军位置在 5 个阶段关键帧间线性插值,不是切帧),自动播放 0.5× / 1× / 2× 三档倍速
- **数据规模相当**:50+ 真实地名(遵义、桐梓、土城、扎西、皎平渡…)、5 个红军番号 + 13 个国军番号、5 阶段

> 适合需要快速部署、低流量场景或希望专注于路径动画本身的演示场景;若需要真实地理底图、3D 沙盘或严谨 schema 校验,使用本仓 `four-crossings-cc`。
